...
def beam_search_dual_v2(model, src_seq, src_mask, device, max_node=10,
                     top_node=10, max_len=200, init_state=[1], len_norm=False, lm=None, lm_scale=0.5, alpha=1.0):
    batch_size = src_seq.size(0)
    ...
    for j in range(top_node):
        ...
        for step in range(max_len):
            ...
            for k in range(l):
                ...
                dec_out = torch.log_softmax(dec_out[:, -1, :].squeeze(1), -1)

                probs, tokens = dec_out.topk(max_node, dim=1)
                probs, tokens = probs.cpu().numpy(), tokens.cpu().numpy()

                for beam, prob, token in zip(beams_ex, probs, tokens):
                    beam.advance(k, prob, token)
        ...
        for hypo, beam, score in zip(hypos, beams_ex, scores):
            hypo.append((beam.hypo(0), beam.score(0) + alpha * score))
...
def beam_search_dual(model, src_seq, src_mask, device, max_node=10,
        top_node=10, max_len=200, init_state=[1], len_norm=False, lm=None, lm_scale=0.5):
    batch_size = src_seq.size(0)
    enc_out, enc_mask = model.encode(src_seq, src_mask)[0:2]
