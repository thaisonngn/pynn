...
from pynn.decoder.s2s import beam_search_dual_v2
...
parser.add_argument('--top-size', help='top size', type=int, default=0)
...
if __name__ == '__main__':
    ...
    top_size = args.beam_size if args.top_size==0 else args.top_size
    decode_func = beam_search_dual_v2
    with torch.no_grad():
        while True:
            seq, mask, utts = reader.read_batch_utt(args.batch_size)
            if not utts: break
            seq, mask = seq.to(device), mask.to(device)
            hypos = decode_func(model, seq, mask, device, args.beam_size, top_size,
                                        args.max_len, len_norm=args.len_norm, lm=lm, lm_scale=args.lm_scale, alpha=args.alpha)
