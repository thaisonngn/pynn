...
from pynn.bin import print_model, distill_s2s_model
...
parser.add_argument('--teacher', help='teacher model path', required=True)
...
def create_model(args, device):
    mdic = torch.load(args.teacher)
    teacher = load_object(mdic['class'], mdic['module'], mdic['params'])
    teacher.load_state_dict(mdic['state'])
    teacher.eval()
...
def train(device, args):
    model, teacher = create_model(args, device)
    print_model(model)
    distill_s2s_model(model, teacher, args, device)
...
def train_distributed(device, gpus, args):
    ...
    distill_s2s_model(model, teacher, args, device, gpus)
