...
def train_epoch(criterion, model, data, opt, device, b_update, b_sync, n_print,
    ...
    for batch_i, batch in enumerate(loader):
        ...
        try:
            ...
            with autocast(enabled=fp16):
                ...
                preds, masks = model(inputs, masks)
...
def eval_epoch(criterion, model, data, device, fp16):
...
def train_model(model, datasets, epochs, device, cfg, fp16=False, dist=False):
    ...
    while epoch_i < epochs:
        ...
        start = time.time()
        tr_loss, tr_accu = train_epoch(criterion, model_opt, tr_data, opt, device, b_update, b_sync,
        ...
        start = time.time()
        cv_loss, cv_accu = eval_epoch(criterion, model_opt, cv_dat, device, fp16=fp16)
