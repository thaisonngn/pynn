...
def cal_loss_trainer(pred, mem_attn_outs, gold, label_mem, label_gate, eps=0.0):
    ...
    mem_attn_outs = [(*pred_to_loss(x[0], label_gate, mask_no_memory),
                      *pred_to_loss(x[0], label_gate, mask_memory),
                      *pred_to_loss(x[1], (label_mem-1).clamp(min=0), mask_memory, eps=eps))
    ...
    loss_attn = torch.stack([x[4] for x in mem_attn_outs]).mean()
    ...
    loss = loss_tok_pred_no_memory/anz_no_memory*(anz_memory+anz_no_memory) + \
           ...
           loss_attn/anz_memory*(anz_memory+anz_no_memory) + \
           loss_gate_no_memory / anz_no_memory * (anz_memory + anz_no_memory) + \
           loss_gate_memory / anz_memory * (anz_memory + anz_no_memory)

    stats = torch.tensor([[y.cpu().item() if not type(y) == int else y for y in x] for x in mem_attn_outs])  # n_dec_layer x 6
...
def printStats(stats, stats2, n_seq=-1, lr=-1, steps=-1, time=-1, train=None):
    ...
    for y in range(stats.shape[0]):
        print(("    After layer %1d: Attention over the memory: ppl:%8.4f,acc:%5.2f\n"+
               "        Gate: No_mem: ppl:%7.4f,acc:%6.2f, Mem: ppl:%7.4f,acc:%6.2f, All: ppl:%7.4f,acc:%6.2f")
              %(y+1,*ppl_acc(stats,stats2,[(y,4)],1),
                    *ppl_acc(stats,stats2,[(y,0)],0),
...
def train_epoch(model, data, opt, eps, device, args, b_update, b_sync, n_print,
    ...
    stats = torch.zeros((args.n_dec, 6))
    p_stats = torch.zeros((args.n_dec, 6))
    ...
    for batch_i, batch in enumerate(loader):
        ...
        try:
            ...
            with autocast(enabled=fp16):
                ...
                if sampling:
                    pred, _, enc_out = model(src_seq, src_mask, tgt_seq, tgt_ids_mem, label_gate, gold)
                ...
                pred, mem_attn_outs, _ = model(src_seq, src_mask, tgt_seq, tgt_ids_mem, label_gate, gold, encoding=not sampling, enc_out=enc_out)
                pred = pred.view(-1, pred.size(2))
                loss, stats_, stats2_ = cal_loss_trainer(pred, mem_attn_outs, gold, label_mem, label_gate, eps=eps)
...
def eval_epoch(model, data, device, args, fp16=False):
    ...
    with torch.no_grad():
        for batch_i, batch in enumerate(loader):
            ...
            # forward
            with autocast(enabled=fp16):
                pred, mem_attn_outs, _ = model(src_seq, src_mask, tgt_seq, tgt_ids_mem, label_gate, gold)
                loss, stats_, stats2_ = cal_loss_trainer(pred, mem_attn_outs, gold, label_mem, label_gate)
