...
from pynn.net.encoder import ConformerNet
from pynn.bin import print_model, train_encoder_model
...
def train(device, args):
    ...
    print_model(model)
    train_encoder_model(model, args, device)
...
def train_distributed(device, gpus, args):
    ...
    if device == 0:
        print_model(model)
    train_encoder_model(model, args, device, gpus)
