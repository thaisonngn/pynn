...
parser.add_argument('--immortal-time', help='immortal size', type=int, default=50)
...
if __name__ == '__main__':
    ...
    stable_time, immortal_time = args.stable_time, args.immortal_time
    ...
    with torch.no_grad():
        while True:
            ...
            for i in range(max(time_len-start-1, 0) // win + 1):
                ...
                enc_out, mask, hypo, score, sth = partial_search(model, src, beam_size, max_len, stable_hypo)
                print(sth)
                ...
                if len(sth) > len(stable_hypo):
                    for j in range(len(stable_hypo), len(sth)):
                        if ep[j-1]*4 + immortal_time > end: break
