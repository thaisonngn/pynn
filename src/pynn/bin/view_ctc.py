...
parser.add_argument('--ctc-model', help='ctc model dictionary', default=None)
...
if __name__ == '__main__':
    ...
    if args.ctc_model is not None:
        mdic = torch.load(args.ctc_model)
        ctc = load_object(mdic['class'], mdic['module'], mdic['params'])
        ctc.load_state_dict(mdic['state'])
        ctc = ctc.to(device)
        ctc.eval()
    else:
        ctc = None
    ...
    with torch.no_grad():
        while True:
            ...
            ctc_logit, tg = ctc(seq)[0].softmax(dim=-1), tgt-2
            probs, tg = ctc_logit.squeeze(0).cpu(), tg.squeeze(0).cpu()
            alg = viterbi_align(torch.log(probs), tg, bs, blank, scale)
