...
def train_epoch(model, data, opt, device, b_update, b_sync, n_print,
                weight_noise=False, grad_clip=0., grad_norm=False, fp16=False):
    ...
    for batch_i, batch in enumerate(loader):
        ...
        try:
            ...
            with autocast(enabled=fp16):
                ...
                pred, mask = model(src_seq, src_mask)
                tgt = tgt_seq[:, :pred.size(1)]
                pred = pred.view(-1, pred.size(2))
                loss, loss_data, n_correct, n_token = cal_ce_loss(pred, tgt)
...
def train_model(model, datasets, epochs, device, cfg, fp16=False, dist=False):
    ...
    while epoch_i < epochs:
        ...
        start = time.time()
        tr_loss, tr_accu = train_epoch(model_opt, tr_data, opt, device, b_update, b_sync, n_print,
                                       weight_noise, grad_norm=grad_norm, fp16=fp16)
